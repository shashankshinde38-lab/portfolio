...
export default function InteractiveTestRunner() {
  ...
  return (
    <div className="w-full glass-card rounded-2xl border border-white/10 overflow-hidden shadow-2xl">
      {/* Terminal Title Bar */}
      <div className="runner-toolbar bg-[#0E1726] px-4 py-3 border-b border-white/10 flex flex-wrap items-center justify-between gap-3">
        ...
        {/* Suite Selector Tabs */}
        <div className="runner-tabs flex items-center gap-1 bg-[#070B14] p-1 rounded-lg border border-white/5">
      ...
      {/* Terminal Screen */}
      <div className="runner-screen p-4 sm:p-6 bg-[#070B14]/90 min-h-[260px] max-h-[360px] overflow-y-auto font-mono text-xs sm:text-sm leading-relaxed space-y-1.5">
        ...
        >
          ...
          {displayedLogs.map((log, index) => (
            ...
            >
              <span className="text-[#94A3B8]/40 select-none" aria-hidden="true">
      ...
      <div className="runner-footer bg-[#0E1726]/80 px-4 py-2.5 border-t border-white/5 flex flex-wrap items-center justify-between gap-2 text-[11px] font-mono text-[#94A3B8]">
        <div className="runner-stats flex items-center gap-4">
          <span>
            STATUS:{" "}
            <strong className={isCompleted ? "text-[#22C55E]" : "text-[#38BDF8]"}>
              {isCompleted ? "PASSED" : isRunning ? "RUNNING" : "STANDBY"}
            </strong>
          </span>
          <span>
            COVERAGE: <strong className="text-[#F8FAFC]">100%</strong>
          </span>
          <span>
            ASSERTIONS: <strong className="text-[#22C55E]">24/24 PASS</strong>
          </span>
        </div>
        <div className="text-[#38BDF8]">STACK: Playwright · Selenium · JMeter · Postman</div>
      </div>
    </div>
  );
}
